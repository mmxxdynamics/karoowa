//! `karoowa agent` — run AI agents in-process.

use clap::{Args, Subcommand};
use karoowa_agents::{
    create_provider, run_agent_step, Agent, CliDevAgent, MonitoringAgent, OnboardingAgent,
    ProviderConfig,
};
...
enum AgentCommand {
    ...
    Onboard,
    /// Run the Monitoring Agent — reports on node health
    Monitor {
...
pub async fn run(args: AgentArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        ...
        model = config.model,
        "starting agent (in-process mode)"
    );
