...
# `genesis generate` writes keys 0600, owned by whoever ran it. They are
# bind-mounted into containers running as `nonroot` (uid 65532), which is
# neither the owner nor in the group, so the node cannot read the key and
# crash-loops. Relax to 0644 *for the devnet only*: these are throwaway keys for
# a local test network, regenerated by deleting them.
#
# Deliberately outside the generation guard above — keys may already exist at
# 0600 from a previous run or from following the README manually, and in that
